...
// Appelé dans le constructeur
    ...
    _carre(move){
        ...
        for(let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                if(this._board[i][j]._color === move._color){
                    list.push([i,j]);
                }
            }
        }
        ...
        for (let k =0; k<list.length-2; k++){
            ...
            if(list[k]._abs+y >= 8 || list[k]._abs+y < 0 || list[k]._ord-x >= 8 || list[k]._ord-x < 0){
                if(this._board[list[k]._abs+y][list[k]._ord-x]._color === move._color){
                    if(list[k]._abs+x >= 8 || list[k]._abs+x < 0 || list[k]._ord+y >= 8 || list[k]._ord+y < 0){
                        if(this._board[list[k]._abs+x][list[k]._ord+y]._color === move._color){
                            return true;
                        }
                    }
                }
